    ...
    module Fixtures
      class Origin
        ...
        def initialize(source, target, prod_ids)
          ...
          target.load('origin.schema.postgres.sql')
        end
        ...
        def extract_mf(locale)
          mf_ids = target.select_values([%(SELECT "MfID" FROM cds_prod WHERE "ProdID" IN (?)), prod_ids])
          table_name = "cds_voc#{locale}"
          rows = source.select_rows([%(SELECT * FROM #{table_name} WHERE "ID" IN (?) ORDER BY "ID"), mf_ids])
          rows.each { |row| target.insert(table_name, row) }
        end
